...
- **`/inventory`** — Rally's private lighting-inventory app: the single-file HTML from `Rally Inventory/rally_lighting_inventory.html`, copied to `public/inventory/index.html` and served at `/inventory` via a `beforeFiles` rewrite in `next.config.mjs`. **Cloud sync (2026-09-16):** the app mirrors its whole state to `/inventory/api` (`src/app/inventory/api/route.ts`, GET/PUT) → ONE private JSON blob `inventory/state.json` in the Vercel Blob store **rally-inventory** (`store_rDxIQipeqdQct3CO`, access: private; env `BLOB_READ_WRITE_TOKEN`). Newest `updatedAt` wins; PUT returns 409 when the cloud copy is newer; `localStorage` (`rally_v3` + `rally_v3_meta`) stays as the offline cache; the app pulls on boot/focus/45s. The source of truth for the app file is `Rally Inventory/rally_lighting_inventory.html` — edit there, then copy to `public/inventory/index.html`. Disallowed in robots.
